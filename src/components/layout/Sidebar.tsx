...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex lg:flex-col lg:w-56 lg:fixed lg:inset-y-0 lg:z-20 lg:border-r lg:border-white/[0.04] lg:bg-[#09090B] lg:pt-16">
      <nav className="flex-1 px-3 py-5 space-y-0.5">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`transition-colors ${isActive ? "text-[#6366F1]" : "text-white/30 group-hover:text-white/50"}`}>
                {item.icon}
              </span>
      ...
      <div className="px-3 py-4 border-t border-white/[0.04]">
        <Link
          href="/profile"
          className="group flex items-center gap-3 px-3 py-2.5 rounded-xl text-white/40 hover:text-white/70 hover:bg-white/[0.03] transition-all duration-150"
        >
